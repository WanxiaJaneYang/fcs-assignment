...
public class Main{
	...
	//declare a method that receives int and returns String
	{
		...
		//calculate the answer prepared for input		

		if((number_of_50>=0)&&(number_of_20>=0))
		{
			if (remainder == 0)
			//if the money can be withdrew
			{
			result="Here is "+ number_of_50+" $50 notes and "+number_of_20+" $20 notes.";
			}
			else{
			...
			if(remainder==0)
			{
			number_of_50= Math.abs(number_of_50);
			number_of_20= Math.abs(number_of_20);
			result="You need to deposit "+ number_of_50+" $50 notes and "+number_of_20+" $20 notes.";
			}
			else{
			result = "Sorry, the value you input cannot be deposit.";
			}
		}
